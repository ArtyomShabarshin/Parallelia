...
namespace Parallelia
{
	...
	private:
		...
		virtual Concurrency::task_group& DoCompletion() { return m_coreimpl->Completion(); }
		...
		virtual size_t  DoCapacityFactor() const { return m_coreimpl->CapacityFactor(); }

		std::shared_ptr<ParalleliaCore::OutputDataFlowBlockCoreImplement<T> > m_coreimpl;

		
	};

	template<typename T> 
	ActionBlock<T>::ActionBlock(F func) :  m_coreimpl(new ParalleliaCore::OutputDataFlowBlockCoreImplement<T>(func, DataflowBlockOptions::Default()))
	{}


	template<typename T> 
	ActionBlock<T>::ActionBlock(F func, const DataflowBlockOptions& options) : m_coreimpl(new ParalleliaCore::OutputDataFlowBlockCoreImplement<T>(func, options))
	{}
